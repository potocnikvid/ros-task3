...
class face_localizer:
    def __init__(self):
        ...
        self.depth_subscriber = message_filters.Subscriber('/camera/depth_registered/image_raw', Image)
                    ...
                    if not recognized:
                        ...
                        if self.confirming_rep == 5:
                            ...
                            if success_num >= 4:
                                ...
                                face = MoveBaseGoal()  
                                face.target_pose.header.frame_id = "map"
                                face.target_pose.pose.orientation.w = 1.0
                                # face.target_pose.pose.position.x = (pose.position.x + robot_position.x)/2
                                # face.target_pose.pose.position.y = (pose.position.y + robot_position.y)/2
                                x1 = pose.position.x 
                                y1 = pose.position.y
                                x0 = robot_position.position.x
                                y0 = robot_position.position.y
                                # dn = np.sqrt((x1 - x0)**2 + (y1 - y0)**2)
                                t = 2/3
                                face.target_pose.pose.position.x = (1 - t)*x0 + t*x1
                                face.target_pose.pose.position.y = (1 - t)*y0 + t*y1                        
                                face.target_pose.header.stamp = rospy.Time()
                                self.ac.send_goal(face)
